    ...
    <div class="card bg-primary mb-3 bg-img" style="background-image: url('{{ asset('assets/img/core-img/1.png') }}')">
      <div class="card-body direction-rtl p-4">
        <div class="row g-3">
          <div class="col-6">
            <div class="feature-card mx-auto text-right">
              ...
              <h6 class="text-white">
                <b>Rp. {{ number_format($totalSaldo ?? 0, 0, ',', '.') }}</b>
              </h6>
            </div>
          </div>
        ...
        <small class="mb-0 text-white">
         Selamat Datang <b>{{ $wali['nama_wali'] ?? '' }}</b> — akun santri <b>{{ $child['nama'] ?? '' }}</b>
        </small>
      </div>
    </div>
  </div>
      ...
      <div class="tiny-slider-two">
        <div>
          <div class="single-hero-slide bg-img bg-overlay"
               style="background-image: url('{{ asset('assets/img/bg-img/3.jpg') }}'); height: 120px;">
        ...
        <div>
          <div class="single-hero-slide bg-img bg-overlay"
               style="background-image: url('{{ asset('assets/img/bg-img/6.jpg') }}'); height: 120px;">
            <div class="h-100 d-flex align-items-center">
              <div class="slide-text">
...
<div class="container">
  <div class="card shadow-sm border-0 rounded-4 overflow-hidden mt-3">
    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
      ...
      <a href="{{ route('tagihan.index') }}" class="text-primary fw-bold text-decoration-none">
          ...
          <li class="list-group-item d-flex justify-content-between align-items-center py-3">
            <a href="{{ route('tagihan.detail', ['id' => $tagihan['id']]) }}"
              ...
              <div class="flex-grow-1">
                <strong>{{ $tagihan['nama_tagihan'] ?? 'Tagihan' }}</strong>
                <small class="text-muted d-block">{{ $tagihan['periode'] ?? 'Periode berjalan' }}</small>
              </div>
              <span class="badge bg-danger text-white p-2 rounded-pill">
